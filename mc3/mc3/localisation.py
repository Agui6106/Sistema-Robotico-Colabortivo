...
class FramePublisher(Node):
    ...
    def timer_cb(self):
        if self.wr and self.wl:
            ...
            '''
            msg = f"""
            ========= POSE DATA RESPECT TO: "odom" =========
            Pos X       : {self.x:.4f} m
            Pos Y       : {self.y:.4f} m
            Theta       : {self.theta:.4f} rad 

            ================ VELOCITY DATA ================
            Linear Vel  : {self.v:.4f} m/s
            Angular Vel : {self.w:.4f} rad/s

            Wheel L     : {self.wl:.4f} rad/s
            Wheel R     : {self.wr:.4f} rad/s
    ...
    def create_odom_msg(self):
        ...
        odom_msg.child_frame_id = 'base_footprint' # Set the child frame id 
