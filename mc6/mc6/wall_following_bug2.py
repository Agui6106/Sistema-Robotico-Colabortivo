...
class LaserScanSub(Node): 
    def __init__(self): 
        ...
        self.m = 0.0 # Slope of the line from robot to goal
        self.b = 0.0 # Y-intercept of the line from robot to goal
    ...
    def timer_callback(self): 
        if self.lidar.ranges:
            ...
            d_line = abs(self.m * self.xr - self.yr + self.b) / np.sqrt(self.m**2 + 1)
            ...
            print(f"Distance to line: {d_line}")
            if self.d_leave < abs(self.d_hit - 0.15) and d_line < 0.2:
    ...
    def set_point_cb(self, set_point_msg):
        ...
        self.m = (self.yg - self.yr) / (self.xg - self.xr) # Slope 
        self.b = self.yr - self.m * self.xr # Y-intercept
